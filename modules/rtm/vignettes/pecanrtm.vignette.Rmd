...
if(file.exists("inversion.output.rds")){
    ...
    inversion.output <- invert.auto(observed = observed,
                                    ...
                                    quiet = TRUE,
                                    parallel = FALSE)  
    saveRDS(inversion.output, "inversion.output.rds",compress="gzip")
}
```
...
par(mfrow=c(1,1))
samples.mat <- as.matrix(inversion.output$samples)[-seq(2000,1),1:3] 
pairs(samples.mat, pch=".")  

means <- head(unlist(inversion.output$results[grep("mu", names(inversion.output$results))]), 3)   
prospect.sim <- prospect(means, 5)[,1]  

plot(wl, observed, type='l', col=1, xlab="wavelength (nm)", ylab="reflectance")
lines(wl, prospect.sim, col=2)
legend("topright", c("observed", "predicted"), lty=1, col=1:2)

rm(samples.mat, means, prospect.sim)  # Free memory  
gc()
