...
approx.posterior <- function(trait.mcmc, priors, trait.data = NULL, outdir = NULL, filename.flag = "") {
  PEcAn.logger::logger.info("DEBUG: Entering approx.posterior")
  ...
  for (trait in names(trait.mcmc)) {
    PEcAn.logger::logger.info(paste("DEBUG: Processing trait:", trait))
    ...
    if (pdist %in% "beta") {
      ...
      PEcAn.logger::logger.info("DEBUG: Fitting zerobound/unif")
      ...
      PEcAn.logger::logger.info("DEBUG: Fitting normal (default)")
      ## default: NORMAL
  ...
  PEcAn.logger::logger.info("DEBUG: Exiting approx.posterior")
  return(posteriors)
} # approx.posterior
